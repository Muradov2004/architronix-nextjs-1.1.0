...
const TeamCardOne = ({ img, social_link, name, position, text_muted, cardVariants, prantCalss,check = true }) => {
    ...
    >
      <div className='relative group hover-underline'>
        ...
        <div className='bg-secondary  absolute left-0 bottom-[-10%] min-w-[295px] transition-all duration-500 group-hover:min-w-full'>
          ...
          <Link href={"/about-us"} className='flex items-center justify-between px-[27px] pt-[9px] pb-[18px]'>
            <label>
            ...
            {check && <small className='text-primary-foreground mt-2.5 flex items-center gap-3 transition-all duration-500 opacity-0 group-hover:opacity-100'> <RightArrow width={"35"} height={"21"} /> </small>}
          </Link>
        </div>
      </div>
    </motion.div>
